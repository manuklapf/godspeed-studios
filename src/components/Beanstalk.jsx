...
function useWaterDropAnimation(root) {
  ...
  useFrame(({ clock, pointer, camera }) => {
    ...
    dropNodes.current.forEach((node) => {
      ...
      node.position.x = base.x + Math.sin(t * 0.31 + phase * 1.3) * 0.003
      node.position.z = base.z + Math.cos(t * 0.37 + phase * 0.9) * 0.003
      ...
      {hoveredData && (
        ...
        >
          <div className="speech-bubble">
            ...
            <strong className="speech-bubble-title">
              {hoveredData.bubbleLabel || hoveredData.title}
            </strong>
          </div>
        </Html>
      )}
...
function makeRustTexture() {
  return makeRustTextures().colorTex
}
...
export function ProceduralBeanstalk() {
  ...
  const vineGeo = useMemo(() => {
    ...
    return new THREE.TubeGeometry(vc, 120, 0.1, 4, false)
  }, [])
