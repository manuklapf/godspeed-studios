...
function GashaponModel({
  ...
  const focusCameraOnPaper = (planeName) => {
    ...
    if (approachDirection.y < 0) {
      approachDirection.applyAxisAngle(pitchAxis, THREE.MathUtils.degToRad(120))
    }
  ...
  useEffect(() => {
    ...
    /* ── Anisotropic filtering on all GLB textures ────────── */
    ...
    const onFinished = (e) => {
      ...
      if (e.action === actionsRef.current[selectedPlaneConfig.actionName]) {
        ...
        const danceLoopName = selectedPlaneConfig.danceLoopName
        const danceAction = actionsRef.current[danceLoopName]
        if (danceAction) {
  ...
  const handleClick = (e) => {
    ...
    if (findNamed(e.object, "Turner") && !turnerClickedRef.current) {
      ...
      if (audioRef.current && selectedPlaneConfig.audioSrc) {
        ...
        fadeAudio(audioRef.current, 0.5, 1000)
      }
...
export default function FashionCampaignPage() {
  ...
  useEffect(() => {
    if (!muted && !turnerClicked) {
      ...
      fadeAudio(menuAudioRef.current, 0.5, 800)
      ...
      <section className="fc-page">
        ...
        <div className="fc-viewer-wrap">
          ...
          <div className="fc-canvas-wrap">
            ...
            {showStart && modelLoaded && (
              <div className="fc-start-overlay">
                <div className="fc-start-window">
                  <h2 className="fc-start-title">
                    Choose Your Random
                    <br />
                    Tattoo Character
                  </h2>
            ...
            {!turnerClicked && !showNextBtn && !showStart && (
              <CanvasOverlay
